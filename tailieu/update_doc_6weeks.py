...
def update_document_to_6_weeks(doc_path):
    print(f"Updating {doc_path}...")
    doc = docx.Document(doc_path)
    
    # 1. Update text paragraphs mentioning 9 weeks
    ...
    # 2. Replace Table 1 (Project Plan) with 6 weeks
    plan_data = [
        # W1
    ...
    for r_idx, (w_time, task, member, note) in enumerate(plan_data):
        ...
        if w_time.startswith("Tuần"):
            bg_color = "EFF6FF"  # Highlight week header row
    ...
    doc.save(doc_path)
    print(f"Successfully saved {doc_path} with 6-week plan!")
